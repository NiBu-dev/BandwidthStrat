...
strats_cc=[x.split('.')[0] for x in strats if x.split('.')[-1]=="cc"]
#strats_py=['tut_mymy']
#os.system("export LD_LIBRARY_PATH="+folder)
#libc = ctypes.cdll.LoadLibrary(ctypes.util.find_library('c'))
strats_cc = []
for strat in strats_cc:
    os.system("clang++ -o "+folder+"/"+strat+".os -c -std=c++11 -Wall -fvectorize -fslp-vectorize -fcolor-diagnostics -O2 -fPIC "+folder+"/"+strat+".cc")
    os.system("clang++ -o "+folder+"/"+strat+".so -shared "+folder+"/"+strat+".os")
...
for _ in range(10):
    ...
    for strat in strats_cc:
        mod = ctypes.cdll.LoadLibrary(folder+"/"+strat+'.so')
#        mod.function.restype=ndpointer(dtype=ctypes.c_int, shape())
#        mod=importlib.import_module(strat)
        sim=Sim(max(0,mod.init_bw()))
        for e in events:
            [time,claim]=e
            sim.process(time)
            queue=sim.get_queue_list(time)
            pyarr1=[ty2int(x[0]) for x in queue]
            pyarr2=[x[1] for x in queue]
            arr1 = (ctypes.c_int * len(pyarr1))(*pyarr1)
            arr2 = (ctypes.c_int * len(pyarr2))(*pyarr2)
#            mod.decision.restype=ctypes.POINTER(ctypes.c_int * (len(queue)+1))
            mod.decision.restype=ndpointer(dtype=ctypes.c_int, shape=(len(queue)+1,))
            lista=mod.decision(ty2int(claim), len(queue), arr1, arr2)
#            lista=mod.decision(ty2int(claim), [ty2int(x[0]) for x in sim.get_queue_list(time)], [x[1] for x in sim.get_queue_list(time)])
            print 'lista', len(queue), pyarr1, lista
            new=lista[-1]
            lista=lista[:-1]
            sim.step(lista)
            if new==1:
